...
const sharedComponentRootPath = '../sharedsite/src/components';
...
function watchComponentFactory() {
  ...
  // watchItems(sharedComponentRootPath, writeComponentFactory);
}
...
function writeComponentFactory() {
  ...
  const sharedComponents = getComponentList(sharedComponentRootPath);

  //we need the path to be relative to this file when we access the files using `getComponentList` above, but then we need to replace the relative path
  // for the custom shared components to be the path relative to the final componentFactory.ts file inside /temp output directory so we are swapping this here.
  const repathedSharedComps = sharedComponents.map((comp: ComponentFile) => {
    comp.path = comp.path.replace('../', '../../../');
    return comp;
  });

  //combine shared and any custom defined packages with components
  components.unshift(...packages, ...repathedSharedComps);
